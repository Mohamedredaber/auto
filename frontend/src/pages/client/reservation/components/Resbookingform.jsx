...
export default function ResBookingForm({ car, initialDepart, initialRetour, initialSubtotal }) {
  ...
  if (submitted) {
    return (
      <div className="res-section" style={{ textAlign: "center", padding: "var(--space-10) var(--space-6)" }}>
        <div style={{ width: 56, height: 56, background: "var(--color-success-bg)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto var(--space-4)" }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--color-success)" strokeWidth="2.5">
        ...
        <p style={{ color: "var(--color-text-muted)", fontSize: "var(--text-sm)" }}>
          {car.brand} {car.model} · {fmt(depart)} → {fmt(retour)} · {subtotal.toLocaleString("fr-MA")} MAD
        </p>
      </div>
    );
  }
        ...
        <div className="res-section__body">
          ...
          <div className="res-breakdown">
            <div className="res-breakdown__row">
              <span className="res-breakdown__label">
                Location ({days} jour{days > 1 ? "s" : ""} × {pricePerDay.toLocaleString("fr-MA")} MAD)
              </span>
              <span className="res-breakdown__value">{subtotal.toLocaleString("fr-MA")} MAD</span>
            </div>
            <div className="res-breakdown__row">
              <span className="res-breakdown__label">Frais de service AutoConnect</span>
              <span className="res-breakdown__value--free">Gratuit</span>
            </div>
            <div className="res-breakdown__row">
              <span className="res-breakdown__label">Taxes et frais locaux</span>
          ...
          <div className="res-total">
            ...
            <div className="res-total__col">
              <span className="res-total__amount">{subtotal.toLocaleString("fr-MA")} MAD</span>
          ...
          {/* Error */}
          {error && (
            <div style={{ background: "var(--color-error-bg)", border: "1px solid var(--color-error)", borderRadius: "var(--radius-md)", padding: "var(--space-3) var(--space-4)", marginTop: "var(--space-4)", fontSize: "var(--text-sm)", color: "var(--color-error)" }}>
              {error}
            </div>
          )}

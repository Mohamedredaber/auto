...
export default function StepAgencyInfo({ onBack, onNext }) {
    ...
    const onSubmit = async (values) => {
        // On passe simplement les valeurs à l'étape suivante
        onNext(values);
    };
        ...
        <div className="rp-view">
            ...
            <form onSubmit={handleSubmit} noValidate>
                <div className="rp-row">
                    <InputField label="Prénom" id="aPrenom" name="prenom" placeholder="Mohammed" icon={<Icon.User />} value={values.prenom} onChange={handleChange} error={errors.prenom} />
                    <InputField label="Nom" id="aNom" name="nom" placeholder="El Fassi" icon={<Icon.User />} value={values.nom} onChange={handleChange} error={errors.nom} />
                </div>
                <InputField label="Email" id="aEmail" name="email" type="email" placeholder="contact@agence.ma" icon={<Icon.Mail />} value={values.email} onChange={handleChange} error={errors.email} />
                <InputField label="Téléphone" id="aTel" name="tel" type="tel" placeholder="+212 06 00 00 00" icon={<Icon.Phone />} value={values.tel} onChange={handleChange} />

                <div className="rp-group">
                    <label className="rp-label">Mot de passe</label>
                    <div className="rp-input-wrap">
                        <span className="rp-icon"><Icon.Lock /></span>
                        <PasswordInputInternal id="aPass" name="pass" value={values.pass} onChange={handleChange} error={errors.pass} />
                    </div>
                    <PasswordStrength password={values.pass} />
                    {errors.pass && <span className="rp-errmsg show"><Icon.Error /> {errors.pass}</span>}
                </div>

                <InputField label="Confirmer le mot de passe" id="aPassC" name="passConfirm" type="password" placeholder="••••••••" icon={<Icon.Lock />} value={values.passConfirm} onChange={handleChange} error={errors.passConfirm} />
